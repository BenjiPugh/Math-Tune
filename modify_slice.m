function modified_slice = modify_slice(slice, freq)

    % Align the frequency bands of a signal to 12 tone equal temperament
    % Args:
    %   slice: a normalized column vector representing the discrete fourier
    %          transform at a single point in time.
    ...
    % Reconstitute the modified slice
    modified_slice = subtracted + rearranged;
end